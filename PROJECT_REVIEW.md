...
| 7 | Collision sampling and weapon cadence depend on frame timing | 🟡 Partial | Player bullets use swept segment-vs-sphere tests (`prevPosition` → `position`) against fighters, air units and ground units. The gun cooldown keeps its remainder (≤ 3 shots per frame, no banking). Chase-camera smoothing and game-over orbit speed are time-based. Probes: 60/61/61/61 shots over the same simulated time at 144/60/20/10 fps (previously 20/25/30 at 20/50/60 fps); a bullet that crossed a fighter part between samples hits it. **Not done:** a bounded fixed-step simulation, and swept tests for bombs/missiles (they are slow). |
...
| 16 | Rendering and startup need measured budgets | ⏳ Not started | Needs profiling on real integrated GPUs; headless SwiftShader numbers are not representative. |
