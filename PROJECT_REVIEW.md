...
| 8 | Damage eligibility duplicated and inconsistent | ✅ Fixed | One rule, `canDamageGround(unit, weapon)`, for bullets/bombs/missiles/napalm: dead units, units with a living protector, and bomb-only hangars (README: "Bomb damage only"). An airport's death now *exposes* its turrets as the README describes: they are moved into the scene keeping their world transform, and are no longer silently deleted with it. Probe covers protection before and exposure after. **One pipeline** (`src/combat/hits.js`): bullets, bombs, missiles and napalm all call `beginHits(weapon)` → `damage(target, amount)` → `finish()`. That applies eligibility, the HP change and label for ground, air and legacy-fighter targets, deferred and deduplicated deaths, and one hit marker and sound per shot or explosion. Rewards go through one `awardKill()` (score × streak, XP), and every kill function accepts `{ reward: false }`. Probe `damagePipeline`: a target hit twice in one blast dies and rewards once, rewards per kind match, a hangar ignores missiles but takes bombs, and a no-reward kill leaves the score unchanged. |
...
| 14 | Procedural placement needs validity checks | ✅ Fixed | `world/populate.js`: bounded rejection sampling (200 tries, then warn and skip), base spawn points validated against the actual islet polygon. Islets store their true bounding radius (`boundR`) for quick rejection tests. Tubes regenerate their path (6 tries) instead of forcing the radius above the safe bound. If no safe path fits, that tube is skipped with a console warning; this happened once in a live smoke run, which spawned 5 of 6 tubes. Every run is seeded: `window.__vpSeed` is shown in the debrief, and `?seed=N` or **Replay this map** regenerate the identical world (probe `replay` compares islet layouts). **Footprints and separation:** each base kind has a coast clearance sized to how far its spawner places units (`BASE_RULES` in `world/populate.js`: airbase 120 inland, forward base 90, carrier group 150 and destroyer squadron 60 offshore), and base centres keep ≥ 300 apart. After half the attempts both requirements relax to 60 % rather than skip a base. Probe `placement` replays 8 fixture seeds: all 18 bases placed on every seed, 1 relaxed, 0 skipped, 0 clearance violations, minimum separation 258. |
| 15 | Smaller visual inconsistencies | ✅ Fixed | Rail and sandbag materials are one clone per fence group, so damage tint no longer spreads to other bases. Napalm patches sit just above the islet or water surface they land on. Colour-lines mode (`effects/colorMode.js`) now sets `scene.overrideMaterial` to one shared wireframe material instead of swapping each mesh's material. Objects spawned or removed while the mode is on are covered automatically, and no mesh references or per-mesh materials are kept (probe `colorMode`). Trade-off: colours come from surface normals rather than per-object HSL, and labels render as wireframe too. |
| 16 | Rendering and startup need measured budgets | ✅ Budget met — light budget done | **Tooling** (README → Performance Tooling): in-game profiler (`P` / `?perf`) with per-system CPU, GPU timer queries, draw calls, lights in shaders; `?seed`, `?invulnerable`, `?disable=`; `npm run bench` compares git refs and feature variants, with an optional CPU profile. **Baseline** (Apple M4, ANGLE Metal, uncapped, `hover`, seed 1): 26.8 fps, CPU 36.7 ms/frame (36.1 ms of it three.js render submission, dominated by `uniformMatrix4fv`), GPU 70.7 ms, 931 draw calls, 98k triangles, 50 lights in shaders, world init 27 + 4 ms. The original `edc4f4b` measures 23.0 fps, so the refactor is not a regression. **Attribution:** `?disable=searchlights` → 254 fps (GPU 3.7 ms, CPU 3.5 ms); `?disable=fences` (removes tower lights) → 249 fps; `?disable=labels` → no change. The ~47 PointLights were the bottleneck. **Light budget (done):** searchlights are virtual lights with unlit glow bulbs, and a fixed pool of 4 real PointLights follows the nearest ones (`src/effects/lightBudget.js`). Lights in shaders are now 7 and constant, so destroying a searchlight never recompiles shaders (probe `lightBudget`). Same seed and view: **49.6 → 438.6 fps**, GPU 37.7 → 1.6 ms, CPU render 19.0 → 1.5 ms. The overlay now shows game state, the share of frames that ran gameplay, idle systems and light-budget use. **Not pursued, no measured need:** instancing (~720 draw calls), cheaper scenery materials and tracer pooling. After the light budget the benchmark frame is 1.6 ms GPU and 1.8 ms CPU at 438 fps uncapped, and world init takes ~30 ms. ROADMAP P2–P4 keep these items for weaker hardware, to be checked with `npm run bench`. |
...
| 19 | HUD and session flow usability pass | ✅ Fixed (touch controls deferred) | Minimap sized `min(400px, 42vmin)` (38vmin on narrow screens). Debrief width `min(640px, 94vw)`. Below 900 px wide, reference panels are hidden, the stats bar wraps and the ammo HUD moves to the corner — no panel overlaps at 390×844, 768×1024 or 1440×900. Mouse controls, Esc, V, G and Enter are listed. Memory diagnostics are off by default (M). Enter restarts after game over. **Session flow** (`src/game/session.js`): a start menu (the simulation and weapons wait; it lists controls and best score), a pause menu (Resume · Restart · Settings) and game-over and debrief buttons (Restart · Replay this map). Menus use real buttons, so mouse, keyboard (Tab / Enter / Esc) and gamepad (D-pad, A, B, Start) all work. Focus moves into each menu, the cursor is visible in menus, and dialogs carry `aria-modal`. **Settings** (`src/core/settings.js`, persisted): volume, mute, mouse steering, invert pitch, reference panels. The debrief has a text summary, which is also the chart's accessible name. `prefers-reduced-motion` disables animations. Restart reloads with `?autostart`, which takes ~30 ms of world init; this is simpler than rebuilding the world in place. Probes `menus`, `settings`, `replay`. **Deferred:** touch controls, which need their own design. |
| 20 | Extract modules | ✅ Done | `main.js` split mechanically into 43 native ES modules (`src/config.js`, `state.js`, `core/`, `world/`, `player/`, `entities/`, `combat/`, `effects/`, `game/`, `ui/`, `audio.js`, `input.js`, `ai.js`, `main.js`). Cross-module mutable `let`s moved onto the `state` object; the split was checked for load-order (TDZ) safety and smoke-tested in headless Brave with every weapon/toggle. Later fixes added their own modules: damage rules (`combat/damage.js`), storage (`core/storage.js`), resource ownership (`core/utils.js`). **Entity contract** (`src/entities/contract.js`, JSDoc typedefs): `entityKind`, `entityHp`, `isAlive`, `isHostile` and `entityPosition` over the three unit shapes, plus `missileTargets()` and `nearestAlive()`. The damage pipeline, base alive counts, missile homing and the lock-on reticle use it, so the reticle and real target selection now share one eligibility rule (review §5.2). Air units no longer duplicate HP in `userData.hp`. **Data tables:** ground and air unit stats live in `config.js` (`GROUND_UNIT_TYPES`, `AIR_UNIT_TYPES`), and the five copy-pasted magazine reloads are one table-driven loop. Probe `entityContract`. The three unit shapes themselves are kept, since model and AI code depend on them. |
...
| §5.1 | A clear mission loop | ✅ First version | `src/game/mission.js`: the nearest base not yet eliminated is the objective. A HUD panel shows its name, units left, distance, a bearing arrow and bases conquered. Eliminating every base ends the run as **Mission complete**: +1000 score, gold panel, and the debrief records the outcome. Probe `mission`. No survival mode. |
| §5.2 | Readable threats | ✅ First version | `src/ui/threats.js`: a red screen-edge arc points toward the source of each hit, plus PULL UP / CEILING / BOUNDARY / LOW HP warnings. The lock-on reticle turns blue and reads "NON-HOSTILE" for tankers and balloons, and it uses the same target rule as missiles (#20). Probe `threats` covers both a direct call and a real enemy-bullet collision. |
| §5.3 | Flight and weapon feedback | ✅ First version | An engine hum whose pitch and level follow airspeed, and wind above 60 % speed (`audio.js`). Kills flash the hit marker gold with a distinct confirm sound. Explosions scale by weapon: bomb 1.4×, missile 1.8×, player-hit spark 0.3×. Probe `killFeedback`; sound levels were not checked by ear. |
| §5.4 | Tunable difficulty | ✅ First version | Settings → Difficulty (Easy / Normal / Hard, persisted), from `DIFFICULTY_PRESETS` in `config.js`: enemy bullet damage ×0.5 / 1 / 1.5, time between enemy shots ×1.5 / 1 / 0.7, interceptor waves ×1.5 / 1 / 0.7. Probe `difficulty`. Tuning the progression curve needs playtests. |
| §5.5 | Fast replay | ✅ Done | Seeded maps, Restart and **Replay this map** straight into the flight, and a debrief with outcome, bases, difficulty and seed (see #19, #14). |
...
> **Status (2026-09-15):**
> - Stages 1–5 are complete (see the progress table).
> - Stage 6 content has first versions (§5.1–§5.5 rows). It still needs playtests.
> - The Three.js migration from r128 is intentionally left as its own future change, as recommended above.

### Focused regression coverage worth adding

> **Status:** every bullet below now has a probe in `tests/browser-probes.mjs` (`npm run test:browser`, 27 probes). Not covered: physical controllers, listening tests and long-session performance.
